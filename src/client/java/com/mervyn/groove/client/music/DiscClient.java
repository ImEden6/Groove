...
/** Sends a burn or load request for the held disc and reports the server's answer in chat. */
public final class DiscClient {
    ...
    public static void register() {
        ...
        ClientPlayNetworking.registerGlobalReceiver(DiscPackets.State.TYPE, (packet, context) -> context.client().execute(() -> {
    ...
    public static void use(BlockPos pos) {
        long now = System.nanoTime();
        if (!ClientPlayNetworking.canSend(DiscPackets.Use.TYPE) || now - sent < 250_000_000L
                || pending != null && now - sent < 5_000_000_000L) return;
        pending = UUID.randomUUID(); sent = now;
        ClientPlayNetworking.send(new DiscPackets.Use(pos, pending));
    }
    private DiscClient() {}
}
