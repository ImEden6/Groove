...
public final class GraphCompiler {
    ...
    private LoopPlan build(Graph graph) {
        ...
        for (Graph.Node node : graph.nodes()) {
            ...
            require(node.type() != null && Set.of("tone", "generator/sample", "fast", "euclid", "stack", "output").contains(node.type()), "Unknown node type");
            require(node.type().equals("generator/sample") ? graph.version() == 2 && node.sample() != null : node.sample() == null,
        ...
        for (Graph.Edge edge : graph.edges()) {
            ...
            require(!nodes.get(edge.fromNode()).type().equals("output"), "Output has no outgoing port");
            require(unique.add(edge), "Duplicate edge");
            inputs.get(edge.toNode()).add(edge.fromNode());
        }
        var outputs = nodes.values().stream().filter(n -> n.type().equals("output")).toList();
    ...
    private Compiled visit(String id, int depth) {
        ...
        Set<String> allowed = switch (node.type()) {
            case "tone" -> Set.of("frequency", "gain", "pan", "wave", "cutoffHz");
            case "generator/sample" -> Set.of("pitchRatio", "gain", "pan");
            case "fast" -> Set.of("factor");
            case "euclid" -> Set.of("steps", "pulses", "rotation");
            default -> Set.of();
        };
        ...
        require(node.type().equals("tone") || node.type().equals("generator/sample") ? links.isEmpty()
                : node.type().equals("stack") ? !links.isEmpty() && links.size() <= 16 : links.size() == 1,
                "Wrong input count on " + id);
        List<Compiled> children = new ArrayList<>();
        for (String link : links) children.add(visit(link, depth + 1));
        Compiled result;
        if (node.type().equals("tone")) {
            double frequency = number(node, "frequency", 220);
            require(frequency >= 20 && frequency <= 16000, "Frequency must be 20..16000 Hz");
            int wave = integer(node, "wave", 0, 0, 1);
            double cutoffHz = number(node, "cutoffHz", 20000);
            require(cutoffHz >= 20 && cutoffHz <= 20000, "cutoffHz must be 20..20000 Hz");
            result = new Compiled(Pattern.tone(new Tone(Tone.Wave.values()[wave], frequency,
                    number(node, "gain", .25), number(node, "pan", 0), cutoffHz)), 1);
        } else if (node.type().equals("generator/sample")) {
            result = new Compiled(Pattern.sample(new groove.engine.samples.SampleVoice(node.sample(),
                    number(node, "pitchRatio", 1), number(node, "gain", .8), number(node, "pan", 0))), 1);
        } else if (node.type().equals("stack")) {
            int cost = children.stream().mapToInt(Compiled::cost).sum();
            require(cost <= MAX_EVENTS, "Graph exceeds event budget");
            result = new Compiled(Pattern.stack(children.stream().map(Compiled::pattern).toArray(Pattern[]::new)), cost);
        } else {
            Compiled child = children.getFirst();
            result = switch (node.type()) {
                case "fast" -> {
                    int factor = integer(node, "factor", 2, 1, 16);
                ...
                case "euclid" -> {
                    int steps = integer(node, "steps", 16, 1, 64);
                    int pulses = integer(node, "pulses", 4, 0, steps);
                    int rotation = integer(node, "rotation", 0, -1024, 1024);
