...
public final class Renderer {
    ...
    public void render(float[] output, int offsetFrames, int frames) {
        ...
        for (int i = 0; i < frames; i++, frame++) {
            ...
            for (Voice voice : voices) {
                ...
                double envelope = Math.min(1, Math.min(age / (score.sampleRate() * .005), remaining / (score.sampleRate() * .02)));
                ...
                left += signal * voice.left;
                right += signal * voice.right;
    ...
    private void start(Score.Note note) {
        ...
        if (selected.note != null) stolenVoices++;
        selected.note = note;
        selected.phase = 0;
        selected.increment = note.tone().frequency() / score.sampleRate();
        selected.filter.setLowPass(note.tone().cutoffHz(), score.sampleRate());
