...
| **7** | **Cable Pulse Animation Toggle** | **45** | Low | Accessibility | A persisted Pulse On/Off transport button hides the beat marker on every theme's cables. | Implemented |
...
- **Missing-sample warning on node cards has landed.** A sample node whose asset is missing or has a different hash gets a row of hazard stripes across its card body on every theme, with the reason on hover (`SampleWarnings`). Samples still downloading count as missing until they arrive.
