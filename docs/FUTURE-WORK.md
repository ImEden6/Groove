...
- **Speaker-side block audio routing remains.** Placed editors now own independent persistent draft/committed sessions, ownership, an owner-managed Access UI, and live headphone monitoring of the draft. Speaker binding to a specific editor's committed patch, and retirement of global playback, remain. See [EDITOR-BLOCK-DESIGN.md](EDITOR-BLOCK-DESIGN.md).
