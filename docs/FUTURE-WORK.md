...
- **No live draft-graph monitoring through headphones.** Holding the headphones and right-clicking an editor block now records a persistent, exclusive link to that editor's session (`HeadphoneServer`/`HeadphoneLinks`, stored on the item with its dimension and session, survives logout/relogin and transfers). Continuously rendering that editor's live draft as a private headphone feed once worn, and the 16-block auto-unlink range, are not built yet. See [EDITOR-BLOCK-DESIGN.md](EDITOR-BLOCK-DESIGN.md) for the proposed design.
...
- **Block-session audio routing remains.** Placed editors now own independent persistent draft/committed sessions, ownership, an owner-managed Access UI, and a persistent headphone link (no live audio yet). Speaker connections and retirement of global playback remain. See [EDITOR-BLOCK-DESIGN.md](EDITOR-BLOCK-DESIGN.md).
