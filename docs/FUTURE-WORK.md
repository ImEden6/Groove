...
- **No knob control.** `knob_base.png`/`knob_indicator.png` are generated by
  [generate_atlas.py](../scratch/tools/generate_atlas.py) and packaged, but nothing
  in `EditorState`/`InputController`/`GrooveEditorScreen` draws or drives a knob —
  there is no rotary parameter control yet, only the drag-a-number-value interaction.
...
real sprites — only the knob half of that claim still holds (see above).
