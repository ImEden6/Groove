...
- **No live draft-graph monitoring through headphones.** Holding the headphones and right-clicking an editor block now records a persistent, exclusive link to that editor's session (`HeadphoneServer`/`HeadphoneLinks`, survives logout/relogin). Continuously rendering that editor's live draft as a private headphone feed once worn, and the 16-block auto-unlink range, are not built yet. See [EDITOR-BLOCK-DESIGN.md](EDITOR-BLOCK-DESIGN.md) for the proposed design.
